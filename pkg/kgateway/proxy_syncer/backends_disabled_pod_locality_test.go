...
func TestPerClientClustersTrackSharedClientCapability(t *testing.T) {
	...
	assertState := func(knows bool, altStatName string) {
		require.Eventually(t, func() bool {
			...
			stored := storedClustersForClient(clusters, current[0])
			cluster := stored[backend.ClusterName()]
			return len(stored) == 1 && cluster != nil && cluster.GetAltStatName() == altStatName
	...
	// An unconfirmed sibling changes the shared UCC in place. The client's row
	// must be rebuilt for that exact client version: the overlay branches on
	// KnowsLocalCluster, which is not part of the KRT key.
